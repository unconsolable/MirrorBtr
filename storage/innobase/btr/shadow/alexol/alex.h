...
// node's parent.
  ...
  forceinline data_node_type *get_leaf(T key) const {
    ...
    while (true) {
      ...
      // Handle out of range before cast to int, otherwise if the value is out of the range of int,
      // the behaviour is undefined.
      if (bucketID_prediction < 0) {
        bucketID_prediction = 0;
      } else {
        bucketID_prediction = std::min<double>(bucketID_prediction, node->num_children_ - 1);
      }
      ...
      if (cur->is_leaf_) {
        ...
        if (std::abs(bucketID_prediction - bucketID_prediction_rounded) <=
          ...
          if (bucketID_prediction_rounded <= bucketID_prediction) {
            ...
            if (prev_leaf && prev_leaf->max_key_ >= key) {
            ...
            if (next_leaf && next_leaf->min_key_ <= key) {
        ...
        if (leaf->prev_leaf_ != nullptr) {
          assert(std::floor(leaf->min_limit_) <= key);
        }
        if (leaf->next_leaf_ != nullptr) {
          assert(std::ceil(leaf->max_limit_) >= key);
        }
        return leaf;
      }
    }
      ...
      if (cur->is_leaf_) {
        ...
        if (std::abs(bucketID_prediction - bucketID_prediction_rounded) <=
          ...
          if (bucketID_prediction_rounded <= bucketID_prediction) {
            ...
            auto prev_leaf = leaf->prev_leaf_;
            /*
std::cout << "I enter left " << std::endl;
printf("prev last key is %.20f\n", prev_leaf->last_key());*/
            ...
            auto next_leaf = leaf->next_leaf_;
            // std::cout << "I enter right " << std::endl;
            //  printf("next first key is %.20f\n", next_leaf->first_key());
...
public:
  ...
  P get_payload_last_less_than(const T &key) {
    EpochGuard guard;
  RETRY:
    data_node_type *leaf = get_leaf(key);
    uint32_t version;
    ...
    const int idx = std::min(leaf->lower_bound(key) - 1, leaf->last_pos_);
    if (idx >= 0) {
      P payload = leaf->get_payload(idx);

      if (leaf->test_lock_version_change(version))
        goto RETRY;
      return payload;
    }
    ...
    while (true) {
      ...
      auto original_leaf = leaf;
    PREV_RETRY:
      // When retry happens, original prev_leaf may be splitted,
      // the latch is not released on old nodes before delete.
      leaf = original_leaf->prev_leaf_;
      if (leaf->test_lock_set(version))
        goto PREV_RETRY;
        
      if (leaf->num_keys_ > 0) {
        P payload = leaf->get_payload(leaf->last_pos());
  ...
  // Return true if found payload, else otherwise
  P get_payload_last_no_greater_than(const T &key) {
    // This stats is for bench & profile, but cause concurrency bottleneck
    // stats_.num_lookups++;
    EpochGuard guard;
  RETRY:
    data_node_type *leaf = get_leaf(key);
    ...
    const int idx = std::min(leaf->upper_bound(key) - 1, leaf->last_pos_);
    if (idx >= 0) {
      P payload = leaf->get_payload(idx);
...
public:
  ...
  forceinline bool lock_parent_node(T key,
    ...
    if (cur->is_leaf_) { // seems unneeded
      return false;
    }
  ...
  RETRY:
    ...
    if (key > istats_.key_domain_max_) {
      ...
      if (should_expand_right()) {
        if (superroot_->try_get_write_lock()) {
          if (should_expand_right()) {
            try_expand_root(key, false); // expand to the right
          }
      ...
      if (should_expand_left()) {
        if (superroot_->try_get_write_lock()) {
          if (should_expand_left()) {
            try_expand_root(key, true); // expand to the left
          }
          superroot_->release_write_lock();
        }
      }
    ...
    if (fail) {
      ...
      // if (!root_node_->is_leaf_) {
      //   std::stringstream ss;
      //   auto root_node = static_cast<model_node_type *>(root_node_);
      //   ss << "New root node children count: " << root_node->num_children_ << '\n';
      //   for (int i = 0; i < root_node->num_children_; ++i) {
      //     if (!root_node->children_[i] || !root_node->children_[i]->is_leaf_) {
      //       continue;
      //     }
      //     auto child_node = static_cast<data_node_type *>(root_node->children_[i]);

      //     ss << "New root node child " << i << " local depth: " << (uint16_t)child_node->local_depth_
      //               << " data capacity: " << child_node->data_capacity_
      //               << " [min key, max key]: " << child_node->min_key_ << ' ' << child_node->max_key_
      //               << " [min limit, max limit]: " << child_node->min_limit_ << ' ' << child_node->max_limit_ 
      //               << " predict index " << root_node_->model_.predict_double(child_node->min_key_) << '\n';
      //   }
      //   std::cerr << ss.str();
      // }
      ...
      if (fail == 5) { // Data node resizing
        ...
        // std::endl;
        // 1. Allocate new node
        data_node_type *node = data_node_allocator().allocate(1);
        data_node_type::New_from_existing(node, leaf);
        ...
        if (leaf->local_depth_ != node->local_depth_) {
          // The value may changed when the root node is expanded
          // std::cerr << "New root node local depth: " << (uint16_t)node->local_depth_ << ' ' << (uint16_t)leaf->local_depth_ << std::endl;
        ...
        // TODO: revert to previous thread local counter
        stats_.num_keys++;
        return true;
      }
      ...
      if (fanout_tree_depth == 0) {
        ...
        if (parent == superroot_) {
          update_superroot_key_domain();
          // Leaf node is root node, parent is superroot.
          // Superroot key domain and model will be updated.
          // The minkey and maxkey in leaf node is those of the index.
          assert(istats_.key_domain_min_ == leaf->min_key_);
          assert(istats_.key_domain_max_ == leaf->max_key_);
          node->min_limit_ = node->min_key_;
          node->max_limit_ = node->max_key_;
        }
  ...
  void create_two_new_data_nodes_without_parent(data_node_type *old_node,
                                                bool reuse_model,
                                                data_node_type *&split_left,
                                                data_node_type *&split_right,
                                                bool verbose = false) {
    ...
    int right_boundary = old_node->lower_bound(mid_limit);
    data_node_type *left_leaf = bulk_load_leaf_node_from_existing(
        ...
        append_mostly_left && appending_left_fit);
    ...
    split_left = left_leaf;
    split_right = right_leaf;
  ...
  void split_sideways_downwards_without_parent(
    ...
    if (used_fanout_tree_nodes.empty()) {
      ...
      create_two_new_data_nodes_without_parent(leaf, reuse_model, left_leaf,
                                               right_leaf);
    } else {
      create_new_data_nodes_without_parent(leaf, used_fanout_tree_nodes);
    }
  ...
  RETRAVEL:
    ...
    if (parent == superroot_) {
      ...
      // Remove previously created data nodes
      for (fanout_tree::FTNode &tree_node : used_fanout_tree_nodes) {
        data_node_type *child_node =
            reinterpret_cast<data_node_type *>(tree_node.data_node);
        data_node_allocator().deallocate(child_node, 1);
      }
      // Recreate fanout tree nodes
      used_fanout_tree_nodes.clear();
      assert(experimental_params_.splitting_policy_method == 1);
      fanout_tree::find_best_fanout_existing_node_without_parent<T, P>(
                leaf, stats_.num_keys, used_fanout_tree_nodes, 2);
      // Recreate data nodes
      if (used_fanout_tree_nodes.empty()) {
    ...
    if (should_split_downwards) {
      ...
      if (fanout > repeats) {
        if (!parent->promote_from_read_to_write()) {
          // std::cout << "Promote fails..." << std::endl;
          parent->release_read_lock();
          goto RETRAVEL;
        }
        ...
        new_node->expand(parent, fanout_tree_depth - compute_duplication);
...
*/
  ...
  // a new root node.
  void try_expand_root(T key, bool expand_left) {
    ...
    // Lock the outermost node, avoid change to parent
    if (!outermost_node->try_get_lock()) {
      return;
    }

    // Read lock parent node, for updating pointer to `outermost_node`
    std::vector<TraversalNode> traversal_path;
    if (!lock_parent_node(key, &traversal_path, outermost_node)) {
      outermost_node->release_lock();
      return;
    }
    model_node_type *outermost_node_parent = traversal_path.back().node;

    auto root = static_cast<model_node_type *>(root_node_);
    if (root != outermost_node_parent) {
      if (!root->try_get_write_lock()) {
        outermost_node_parent->release_read_lock();
        outermost_node->release_lock();
        return;
      }
    } else {
      if (!root->promote_from_read_to_write()) {
        outermost_node_parent->release_read_lock();
        outermost_node->release_lock();
        return;
      }
    ...
    // TODO: fix error expansion factor
    if (expansion_factor <= 1) {
      root->release_write_lock();
      if (outermost_node_parent != root) {
        outermost_node_parent->release_read_lock();
      }
      outermost_node->release_lock();
      return;
    ...
    // lock prev leaf, then outermost_node
    data_node_type *prev_leaf = nullptr;
    if (!expand_left) {
      do {
        prev_leaf = outermost_node->prev_leaf_;
        if (prev_leaf == nullptr)
          break;
        if (prev_leaf->try_get_link_lock()) {
          if (prev_leaf ==
              outermost_node->prev_leaf_) { // ensure to lock the correct node
            break;
          } else {
            prev_leaf->release_link_lock();
          }
    ...
    outermost_node->get_link_lock();
    // Do not release lock on old data node & root node, force retry
    ...
    // adapted from AlexModelNode::expand()
    new_root->lock_ = root->lock_;
    new_root->is_leaf_ = false;
    new_root->is_obsolete_ = false;
    ...
    bool create_new_root_node = false;
    if (static_cast<size_t>(root->num_children_) * expansion_factor <=
      ...
      int log2_expansion_factor = log_2_round_down(expansion_factor);
      ...
      const AlexNode<T, P> *prev_node = nullptr;
      for (int i = 0; i < root->num_children_; i++) {
        new_children[copy_start + i] = root->children_[i];
        // Root node expansion does not change duplication factor,
        // so the local depth needs to update
        if (root->children_[i]->is_leaf_ && root->children_[i] != prev_node) {
          root->children_[i]->local_depth_ += log2_expansion_factor;
          prev_node = root->children_[i];
        }
      }
    ...
    if (expand_left) {
      ...
      for (int i = new_nodes_end; i > new_nodes_start; i -= n) {
        ...
        new_node->min_limit_ = (i - n - new_root->model_.b_) / new_root->model_.a_;
        new_node->max_limit_ = (i - new_root->model_.b_) / new_root->model_.a_;
        if (next) {
      ...
      for (int i = new_nodes_start; i < new_nodes_end; i += n) {
        ...
        new_node->level_ = static_cast<short>(new_root->level_ + 1);
        new_node->local_depth_ = new_local_depth;
        new_node->min_limit_ = (i - new_root->model_.b_) / new_root->model_.a_;
        new_node->max_limit_ = (i + n - new_root->model_.b_) / new_root->model_.a_;
        if (prev) {
    ...
    // node.
    data_node_type *new_outermost_node = data_node_allocator().allocate(1);
    data_node_type::Copy_from_existing(new_outermost_node, outermost_node);

    if (expand_left) {
      new_outermost_node->erase_range(new_domain_min, istats_.key_domain_min_);
      auto last_new_leaf =
          static_cast<data_node_type *>(new_root->children_[new_nodes_end - 1]);
      new_outermost_node->prev_leaf_ = last_new_leaf;
      last_new_leaf->next_leaf_ = new_outermost_node;

      auto next_leaf = new_outermost_node->next_leaf_;
      if (next_leaf != nullptr) {
        new_outermost_node->next_leaf_ = next_leaf;
        next_leaf->prev_leaf_ = new_outermost_node;
      }

      if (!create_new_root_node && root == outermost_node_parent) {
        for (int i = new_nodes_end; i < new_root->num_children_; ++i) {
          if (new_root->children_[i] == outermost_node) {
            new_root->children_[i] = new_outermost_node;
          } else {
            break;
          }
        }
        ...
        int bucketID = traversal_path.back().bucketID;
        int repeats =
            1 << (log_2_round_down(outermost_node_parent->num_children_) - outermost_node->local_depth_);
        int start_bucketID =
            bucketID - (bucketID % repeats); // first bucket with same child
        int end_bucketID =
            start_bucketID + repeats; // first bucket with different child
        ...
        for (int i = start_bucketID; i < end_bucketID; ++i) {
          assert(outermost_node_parent->children_[i] == outermost_node);
          outermost_node_parent->children_[i] = new_outermost_node;
        }
      }
    } else {
      new_outermost_node->erase_range(istats_.key_domain_max_, new_domain_max,
                                  true);
      auto first_new_leaf =
          static_cast<data_node_type *>(new_root->children_[new_nodes_start]);
      new_outermost_node->next_leaf_ = first_new_leaf;
      first_new_leaf->prev_leaf_ = new_outermost_node;

      if (prev_leaf) {
        prev_leaf->next_leaf_ = new_outermost_node;
        new_outermost_node->prev_leaf_ = prev_leaf;
      }

      if (!create_new_root_node && root == outermost_node_parent) {
        for (int i = new_nodes_start - 1; i >= 0; i--) {
          if (new_root->children_[i] == outermost_node) {
            new_root->children_[i] = new_outermost_node;
          } else {
            break;
    ...
    new_outermost_node->release_link_lock();
    new_outermost_node->release_lock();

    istats_.key_domain_min_ = new_domain_min;
    istats_.key_domain_max_ = new_domain_max;

    // std::stringstream ss;
    // ss << "New root node children count: " << new_root->num_children_ << '\n';
    // for (int i = 0; i < new_root->num_children_; ++i) {
    //   if (!new_root->children_[i] || !new_root->children_[i]->is_leaf_) {
    //     continue;
    //   }
    //   auto child_node = static_cast<data_node_type *>(new_root->children_[i]);

    //   ss << "New root node child " << i << " local depth: " << (uint16_t)child_node->local_depth_
    //             << " data capacity: " << child_node->data_capacity_
    //             << " [min key, max key]: " << child_node->min_key_ << ' ' << child_node->max_key_
    //             << " [min limit, max limit]: " << child_node->min_limit_ << ' ' << child_node->max_limit_ 
    //             << " predict index " << new_root->model_.predict_double(child_node->min_key_) << '\n';
    // }
    // std::cerr << ss.str();

    root_node_ = new_root;
    update_superroot_pointer();
    if (create_new_root_node) {
      root->release_write_lock();
    } else {
      safe_delete_node(root);
    }
    safe_delete_node(outermost_node);
  }
...
public:
  ...
  int erase(const T &key) {
    ...
    if (ret_flag == 2) {
      ...
      // 2. Resizing
      node->resize_from_existing(leaf, data_node_type::kMaxDensity_);
      ...
      safe_delete_node(leaf);
      // thread_local int resize_counter(0);
      // resize_counter++;
      // if (resize_counter % 10000 == 0) {
      //   std::cout << "Now we have resized " << resize_counter << " times"
      //             << std::endl;
      // }
    }
  ...
  public:
    ...
    explicit NodeIterator(const self_type *index)
      ...
      if (cur_node_ && !cur_node_->is_leaf_) {
        ...
        if (node->children_[node->num_children_ - 1]) {
          node_stack_.push(node->children_[node->num_children_ - 1]);
        }
        for (int i = node->num_children_ - 2; i >= 0; i--) {
          if (node->children_[i] && node->children_[i] != node->children_[i + 1]) {
