...
class spin_lock {
 ...
 public:
  ...
  bool try_lock() { return !lock_.exchange(true); }
...
// optimistic lock implementation is based on
// https://github.com/wangziqi2016/index-microbench/blob/master/BTreeOLC/BTreeOLC_child_layout.h
struct OptLock {
  std::atomic<uint64_t> typeVersionLockObsolete{0b100};

  OptLock() = default;
  OptLock(const OptLock &other) { typeVersionLockObsolete = 0b100; }

  uint64_t get_version_number() { return typeVersionLockObsolete.load(); }

  bool isLocked(uint64_t version) { return ((version & 0b10) == 0b10); }

  bool isLocked() { return ((typeVersionLockObsolete.load() & 0b10) == 0b10); }
  ...
  bool isObsolete() { return (typeVersionLockObsolete.load() & 1) == 1; }
};
