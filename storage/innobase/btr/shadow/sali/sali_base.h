...
class LinearModelInterface{
    ...
    virtual inline void train_two(long double mid1_key,long double mid2_key,long
double mid1_target,long double mid2_target)=0;
...
class LinearModel {
 public:
  ...
  inline void clear() { a = b = 0; }
  inline void train_two(long double mid1_key, long double mid2_key,
                        long double mid1_target, long double mid2_target) {
...
class TwoLinearModel {
 public:
  ...
  TwoLinearModel(T mid, long double a1, long double b1, long double a2,
                 long double b2)
      : mid(mid), a1(a1), b1(b1), a2(a2), b2(b2) {}
  explicit TwoLinearModel(const TwoLinearModel &other)
      : mid(other.mid),
        a1(other.a1),
        b1(other.b1),
        a2(other.a2),
  ...
  inline void clear() { a1 = b1 = a2 = b2 = mid = 0; }
  inline void train_two(long double mid1_key, long double mid2_key,
                        long double mid1_target, long double mid2_target) {
...
class MultiLinearModel {
 public:
  ...
  inline int predict(T key) const { return 0; }

  inline double predict_double(T key) const { return predict_pos(key); }
  inline int predict_pos(T key) {
    ...
    double v2 =
        params[seg_id].a * static_cast<long double>(key) + params[seg_id].b;
    ...
    return segment_offset[seg_id] +
           std::min(segment_size[seg_id] - 1, static_cast<int>(v2));
  }
  ...
  inline void train_two(long double mid1_key, long double mid2_key,
    ...
    /*std::cout<<std::to_string(mid1_target)<<" .."<<
    std::to_string(mid2_target)<<std::endl;
    std::cout<<std::to_string(mid1_key)<<" .."<<
    std::to_string(mid2_key)<<std::endl;
  ...
  inline void print() {
    ...
    for (int i = 0; i < segment_count; i++) {
      std::cout << std::to_string(params[i].a) << " "
                << std::to_string(params[i].b) << std::endl;
