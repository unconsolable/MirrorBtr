...
class OptimalPiecewiseLinearModel {
 private:
  ...
  struct Hull : private std::vector<StoredPoint> {
    ...
    explicit Hull(SY epsilon)
        : std::vector<StoredPoint>(), epsilon(Upper ? epsilon : -epsilon) {}
    ...
    void push(X x, Y y) {
      std::vector<StoredPoint>::emplace_back(StoredPoint{x, y});
    };
  };
 ...
 public:
  ...
  explicit OptimalPiecewiseLinearModel(Y epsilon)
      : epsilon(epsilon), lower(epsilon), upper(epsilon) {
...
class OptimalPiecewiseLinearModel<X, Y>::CanonicalSegment {
 ...
 public:
  ...
  std::pair<long double, long double> get_floating_point_segment(
      const X &origin) const {
...
auto make_segmentation(RandomIt first, RandomIt last, size_t epsilon) {
    ...
    using canonical_segment = typename OptimalPiecewiseLinearModel<key_type,
size_t>::CanonicalSegment; using pair_type = typename std::pair<key_type,
size_t>;
...
size_t make_segmentation_data(size_t n, size_t epsilon, Fin in, Fout out,
lrmodel_type *lrmodel) { if (n == 0) return 0;

  using X = typename std::invoke_result_t<Fin, size_t>::first_type;
  using Y = typename std::invoke_result_t<Fin, size_t>::second_type;
  using canonical_segment = typename OptimalPiecewiseLinearModel<key_type,
size_t>::CanonicalSegment; size_t c = 0; size_t start = 0; auto p = in(0);
  ...
  for (size_t i = 1; i < n; ++i) {
    ...
    bool add_success = opt.add_point(p.first, p.second - start);  // alwayse
start from 0 for each segment if (add_success) keys.push_back(p.first); else {
      canonical_segment cs = opt.get_segment();
      auto[cs_slope, cs_intercept] =
cs.get_floating_point_segment(cs.get_first_x()); lrmodel_type model(cs_slope,
cs_intercept); out(model, keys.begin(), keys.begin(), keys.size(), epsilon);
