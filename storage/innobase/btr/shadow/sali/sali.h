...
static thread_local int thread_num = 0;
// before the thread starts to use sali, it should call this function to setup
// thread id
static void sali_set_thread_num(int num) { thread_num = num; }
static int sali_get_thread_num() { return thread_num; }
  ...
  {                                                                            \
    if (!(expr)) {                                                             \
      fprintf(stderr, "Thread %d: RT_ASSERT Error at %s:%d, `%s` not hold!\n", \
              sali_get_thread_num(), __FILE__, __LINE__, #expr);               \
      exit(0);                                                                 \
    }                                                                          \
  }
...
#define RT_DEBUG(msg, ...)                                                    \
  if (sali_get_thread_num() == 0) {                                           \
    printf(GREEN "T%d: " msg RESET "\n", sali_get_thread_num(), __VA_ARGS__); \
  } else if (sali_get_thread_num() == 1) {                                    \
...
class SALI {
  static_assert(std::is_arithmetic<T>::value, "SALI key type must be numeric.");
 ...
 public:
  ...
  class ThreadSpecificEpochBasedReclamationInformation {
   ...
   public:
    ...
    void scheduleForDeletion(Node *pointer) {
      ...
      std::vector<Node *> &currentFreeList = mFreeLists[mLocalEpoch];
   ...
   private:
    void freeForEpoch(uint32_t epoch) {
      std::vector<Node *> &previousFreeList = mFreeLists[epoch];
      for (Node *node : previousFreeList) {
        if (node->is_two) {
          ...
          tree->pending_two[sali_get_thread_num()].push(node);
        } else {
   ...
   public:
    ...
    void enterCriticalSection() {
      ...
      if (currentMemoryInformation.doesThreadWantToAdvanceEpoch() &&
        ...
        bool ret = mCurrentEpoch.compare_exchange_strong(
            currentEpoch, NEXT_EPOCH[currentEpoch]);
        if (ret) {
          freeForEpochG(
              NEXT_EPOCH[mCurrentEpoch.load(std::memory_order_acquire)]);
        }
      }
    }
    ...
    void freeForEpochG(uint32_t epoch) {
      std::vector<Node *> &previousFreeList = gFreeLists[epoch];
      for (Node *node : previousFreeList) {
  ...
  SALI(double BUILD_LR_REMAIN = 0, bool QUIET = false,
        ...
        memory_budget_(memory_budget) {
    {
      ...
      if (!QUIET) {
        printf("initial memory pool size = %lu\n",
               pending_two[sali_get_thread_num()].size());
      }
    }
    if (USE_FMCD && !QUIET) {
      printf("enable FMCD\n");
      ...
      if (root->items[i].entry_type != 1)
        ...
        for (int j = 0; j < root->items[i].comp.child->num_items; j++) {
          if (root->items[i].comp.child->items[j].entry_type == 2)
            std::cout << "child: "
                      << root->items[i].comp.child->items[j].comp.data.key
                      << root->items[i].comp.child->items[j].comp.data.value
                      << std::endl;
        }
      }
    }
  ...
  int get_real_pos_from_compressed_node(Node *node, int tmp_pos, T key) {
    int lo =
        (tmp_pos) <= (compress_epsilon) ? 0 : ((tmp_pos) - (compress_epsilon));
    int hi = (tmp_pos) + (compress_epsilon) + 2 >= (node->num_items - 1)
                 ? (node->num_items - 1)
                 : (tmp_pos) + (compress_epsilon) + 2;
    while (lo < hi) {
      int mid = lo + (hi - lo) / 2;
      bool needRestart = false;
      ...
      if (node->items[mid].entry_type == 2) {
        ...
        RT_ASSERT(node->items[mid].entry_type == 1 ||
                  node->items[mid].entry_type == 2);
      }
      if (key > mid_key) {
        lo = mid + 1;
      } else {
  ...
  restart:
    ...
    for (Node *node = root;;) {
      ...
      if (node->items[pos].entry_type == 1) {  // 1 means child
        if (node->last_adjust_type == 3) {
          ...
          versionItem =
              node->items[compress_pos].readLockOrRestart(needRestart);
          if (needRestart) goto restart;
          if (node->items[compress_pos].entry_type == 2) {
            ...
            node->items[compress_pos].readUnlockOrRestart(versionItem,
                                                          needRestart);
            if (needRestart) goto restart;
            break;
          } else {
        ...
        if (node->items[pos].entry_type == 0) {  // 0 means empty
          ...
          if (node->items[pos].comp.data.key != key) {
            if (node->last_adjust_type == 3) {
              ...
              if (needRestart) goto restart;
              int compress_pos =
                  get_real_pos_from_compressed_node(node, pos, key);
              if (compress_pos < 0) {
                goto restart;
              }
              versionItem =
              ...
              if (node->items[compress_pos].entry_type == 1) {
                ...
                parent->items[compress_pos].readUnlockOrRestart(versionItem,
                                                                needRestart);
                if (needRestart) goto restart;
                continue;
              } else {
...
#if READ_EVOLVE
    if (likely(path_size <= 3 || !pq_distribution(getGen()) ||
               path[1]->last_adjust_type == 2)) {
      return ret;
    }
    ...
    std::vector<T>
        *keys;  // make it be a ptr here because we will let scan_and_destroy
    ...
    long double speed = (long double)(numKeysCollected - prev_size) /
                        (cur_time - prev_build_time + 1);
  ...
  retryLock:
    if (retryLockCount++) yield(retryLockCount);
  ...
  void bulk_load(const V *vs, int num_keys) {
    ...
    if (num_keys == 2) {
      destroy_tree(root);
      root = build_tree_two(vs[0].first, vs[0].second, vs[1].first,
                            vs[1].second, 0.001, timeSinceEpochNanosec(), 1);
      return;
    }
    ...
    for (int i = 0; i < root->num_items; i++) {
      if (root->items[i].entry_type == 1 &&
        ...
        // std::cout << "push_to_cooling_pool, new_node: " <<
        // root->items[i].comp.child << std::endl;
  ...
  restart:
    ...
    while (true) {
      ...
      {
        return false;
      } else if (node->items[pos].entry_type ==
                 2)  // 2 means existing entry has data already
      {
  ...
  // Find the minimum `len` keys which are no less than `lower`, returns the
  ...
  // Find the maximum key which is no greater than `upper`, returns the number
  // of found keys.
  int find_last_no_greater_than(const T &upper, P *result) {
    EpochGuard guard(this);  // seems that this guard can not be created in a recursively func
    return reverse_range_core<false>(result, 0, root, upper, 1);
  }
  ...
  void print_depth() const {
    ...
    std::cout << "root build_size = " << std::to_string(root->build_size)
              << std::endl;
    std::cout << "root num_items = " << std::to_string(root->num_items)
              << std::endl;
    std::cout << "num_rebuild = " << std::to_string(num_rebuild) << std::endl;
    std::cout << "num_write_probability_trigger = "
              << std::to_string(num_write_probability_trigger) << std::endl;
    std::cout << "num_read_probability_trigger = "
              << std::to_string(num_read_probability_trigger) << std::endl;
    std::cout << "num_lower_64 = " << std::to_string(num_lower_64) << std::endl;
    std::cout << "==============================================" << std::endl;
  }

  long adjust_num() { return num_write_probability_trigger; }
  ...
  std::tuple<long, double, long> depth() {
    ...
    return std::tuple<long, double, long>(
        max_depth, double(sum_depth) / double(sum_nodes), sum_nodes);
  }
...
// return that all specializations of this allocator are interchangeable
  ...
  struct ListNode {
    ...
    ListNode(NodePair val, ListNode *prev, ListNode *next)
  ...
  }, { 0x3fff, 0x1fff, 0xfff, 0x7ff, 0x7ff, 0xfff, 0x1fff, 0x3fff, 0x3fff,
  0xffffffff }, { 0x3fff, 0x1fff, 0xfff, 0x3ff, 0x7ff, 0x7ff, 0xfff, 0x1fff,
  0x3fff, 0x3fff, 0xffffffff }, { 0x3fff, 0x1fff, 0x7ff, 0x1ff, 0x3ff, 0x7ff,
  0x7ff, 0xfff, 0x1fff, 0x3fff, 0x3fff, 0xffffffff }, { 0x3fff, 0x1fff, 0x7ff,
  0xff, 0x1ff, 0x3ff, 0x7ff, 0x7ff, 0xfff, 0x1fff, 0x3fff, 0x3fff, 0xffffffff },
      { 0x3fff, 0x1fff, 0x7ff, 0xff, 0x7f, 0x1ff, 0x3ff, 0x7ff, 0x7ff, 0xfff,
  0x1fff, 0x3fff, 0x3fff, 0xffffffff }, { 0x3fff, 0x1fff, 0x7ff, 0xff, 0x3f,
  0x7f, 0x1ff, 0x3ff, 0x7ff, 0x7ff, 0xfff, 0x1fff, 0x3fff, 0x3fff, 0xffffffff },
      { 0x3fff, 0x1fff, 0x7ff, 0xff, 0x3f, 0, 0x7f, 0x1ff, 0x3ff, 0x7ff, 0x7ff,
  0xfff, 0x1fff, 0x3fff, 0x3fff, 0xffffffff }
  };*/
    ...
    while (exitSignal == 0) {
      ...
      if (exitSignal == 1) {
        while (victim_from_cooling_pool(&nodePair)) {
          ...
          std::vector<T> *keys;
          std::vector<P> *values;
          int numKeysCollected = scan_and_destory_tree2(child, &keys, &values);
          if (numKeysCollected < 0) {
            continue;
          }
          ...
          long double speed = (long double)(numKeysCollected - prev_size) /
                              (cur_time - prev_build_time + 1);
          // bulk_args args={numKeysCollected, speed, cur_time,
          // 1,speed/prev_speed};

          Node *new_node = new_nodes(1);
          OptimalPiecewiseLinearModel<T, P> opt(
              std::numeric_limits<int>::max());
          opt.add_point((*keys)[0], 0);
          int start = 0;
          // int segment_idx=0;
          for (int i = 1; i < numKeysCollected; ++i) {
            bool add_success = opt.add_point(
                (*keys)[i],
                i - start);  // alwayse start from 0 for each segment
            RT_ASSERT(add_success);
          }
          typename OptimalPiecewiseLinearModel<T, P>::CanonicalSegment cs =
              opt.get_segment();
          // pgm auto[cs_slope, cs_intercept] =
          ...
          new_node->conflict_distribution =
              std::bernoulli_distribution(new_node->p_conflict);
      ...
      for (int i = 1; i < numKeysCollected; ++i) {
        bool add_success = opt.add_point(
            (*keys)[i], i - start);  // alwayse start from 0 for each segment
        RT_ASSERT(add_success);
      }
      typename OptimalPiecewiseLinearModel<T, P>::CanonicalSegment cs =
          opt.get_segment();
  ...
  void compress() {
    ...
    std::vector<T> *keys;
    std::vector<P> *values;
    int numKeysCollected = scan_and_destory_tree(child, &keys, &values);
    if (numKeysCollected < 0) {
      return;
    }
    int prev_size = child->build_size;
  ...
  Node *new_nodes(int n) {
    ...
    al[sali_get_thread_num()].field += n * sizeof(Node);
  ...
  Item *new_items(int n) {
    ...
    al[sali_get_thread_num()].field += n * sizeof(Item);
    for (int i = 0; i < n; ++i) {
  ...
  Node *build_tree_two(T key1, P value1, T key2, P value2, long double _speed,
                       long _time, int _type) {
    if (key1 > key2) {
      std::swap(key1, key2);
    ...
    Node *node = NULL;
    if (pending_two[sali_get_thread_num()].empty()) {
      node = new_nodes(1);
      node->is_two = 1;
      node->build_size = 2;
      node->size = 2;
      ...
      node = pending_two[sali_get_thread_num()].top();
      pending_two[sali_get_thread_num()].pop();
    }
    RT_ASSERT(node->build_size == 2);
    RT_ASSERT(node->num_items == 8);
    ...
    const long double mid1_target =
        static_cast<long double>(node->num_items) / 3;
    const long double mid2_target =
        static_cast<long double>(node->num_items) * 2 / 3;

    node->model.train_two(mid1_key, mid2_key, mid1_target, mid2_target);

    /*node->model.a1= node->model.a2= (mid2_target - mid1_target) / (mid2_key -
    mid1_key); node->model.b1= node->model.b2= mid1_target - node->model.a1 *
    mid1_key; node->model.mid=(mid1_key+mid2_key)/2;*/
    ...
    {  // insert key1&value1
      ...
      if (node->items[pos].entry_type != 0) {
        ...
        std::cout << "node->items[pos].entry_type "
                  << node->items[pos].entry_type << std::endl;
      }
  ...
  Node *build_tree_bulk(std::vector<T> *_keys, std::vector<P> *_values,
                        bulk_args &args) {
#if READ_EVOLVE
    args._type = 2;
  ...
  void get_fmcd_output(int L, int N, T *keys, long double *a, long double *b) {
    ...
    if (D * 3 <= N) {
      ...
      *a = 1.0 / Ut;
      *b = (L - (*a) * (static_cast<long double>(keys[N - 1 - D]) +
                        static_cast<long double>(keys[D]))) /
           2;
  ...
  Node *build_tree_bulk_ml(std::vector<T> *_keys, std::vector<P> *_values,
                           bulk_args &args) {
    int _size = args._size;
    long double _speed = args._speed;
    uint64_t _time = args._time;
    int _type = args._type;
    // std::cout<<"build_tree_bulk_fmcd
    // "<<std::to_string(args.ratio)<<std::endl;
    ...
    while (!s.empty()) {
      ...
      RT_ASSERT(end - begin >= 2);
      if (end - begin == 2) {
        Node *_ = build_tree_two((*_keys)[begin], (*_values)[begin],
                                 (*_keys)[begin + 1], (*_values)[begin + 1],
                                 speed, _time, _type);
        memcpy(node, _, sizeof(Node));
        delete_nodes(_, 1);
      } else {
        ...
        node->conflict_distribution =
            std::bernoulli_distribution(node->p_conflict);
        node->build_time = _time;
        node->speed = speed;
        node->last_adjust_type = _type;
        node->num_items = 0;
          ...
          for (int i = 0; i < segment_count; i++) {
            if (segments[i] < 2) {
              long double left_key =
                  (static_cast<long double>(i) - node->model.top_param.b) /
                  node->model.top_param.a;
              long double right_key =
                  (static_cast<long double>(i + 1) - node->model.top_param.b) /
                  node->model.top_param.a;
              ...
              long double tmp_a =
                  (mid2_target - mid1_target) / (mid2_key - mid1_key);
              ...
              long double tmp_b;
              int segment_size =
                  segments[i] * static_cast<int>(BUILD_GAP_CNT + 1);
              RT_ASSERT(segments[i] > 2);
              get_fmcd_output(segment_size, segments[i], keys + offset, &tmp_a,
                              &tmp_b);
              /*node->model.params.emplace_back(tmp_a, tmp_b);
              node->model.segment_size.push_back(segment_size);
              node->model.segment_offset.push_back(segment_offset);*/
        ...
        for (int item_i = PREDICT_POS(node, keys[0]), offset = 0;
          ...
          if (next == offset + 1) {
            ...
            /*if(speed / node->num_items ==0){
              std::cout<<"speed == 0 parent_speed: "<<speed<<"num_items:
            "<<std::to_string(node->num_items)<<std::endl;
            }*/
          }
          if (next >= size) {
            break;
  ...
  Node *build_tree_bulk_two_model(std::vector<T> *_keys,
                                  std::vector<P> *_values, bulk_args &args) {
    int _size = args._size;
    long double _speed = args._speed;
    uint64_t _time = args._time;
    int _type = args._type;
    ...
    while (!s.empty()) {
      ...
      if (end - begin == 2) {
        ...
        if (size == 3) {
          ...
          mid_target =
              static_cast<long double>(node->num_items - 1) *
              (1 - (node->model.mid - static_cast<long double>(keys[0])) /
                       (keys[size - 1] - keys[0]));
        } else {
          int left, right;
          T max = 0;
          ...
          node->model.mid = (static_cast<long double>(keys[left]) +
                             static_cast<long double>(keys[right])) /
                            2;
          int t = -1;
          if (right - left == 1) {
            t = left;
          ...
          mid_target = static_cast<long double>(node->num_items - 1) *
                       ((long double)(t + 1) / (size));
        }

        node->model.a1 = (mid_target) /
                         (node->model.mid - static_cast<long double>(keys[0]));
        node->model.b1 = (mid_target)-node->model.a1 * node->model.mid;
        node->model.a2 =
            (static_cast<long double>(node->num_items) - 1 - mid_target) /
  ...
  /// bulk build, _keys must be sorted in asc order.
  /// FMCD method.
  Node *build_tree_bulk_fmcd(std::vector<T> *_keys, std::vector<P> *_values,
                             bulk_args &args) {
    int _size = args._size;
    long double _speed = args._speed;
    uint64_t _time = args._time;
      ...
      if (end - begin == 2) {
        ...
        // U_T. In fact, it has only a negligible impact of the performance.
        {
          ...
          if (D * 3 <= size) {
            ...
            node->model.b1 =
                (L - node->model.a1 *
                         (static_cast<long double>(keys[size - 1 - D]) +
                          static_cast<long double>(keys[D]))) /
                2;
        ...
        /*std::cout<<"build_tree_bulk_fmcd 1
        "<<std::to_string(node->num_items)<<std::endl;
        std::cout<<"build_tree_bulk_fmcd 2 "<<std::to_string(size *
        static_cast<int>(BUILD_GAP_CNT + 1))<<std::endl;
        std::cout<<"build_tree_bulk_fmcd 3
        "<<std::to_string(static_cast<int>(ratio))<<std::endl;*/
  ...
  Node *build_tree_compress(std::vector<T> *_keys, std::vector<P> *_values,
                            bulk_args &args) {
    int _size = args._size;
    long double _speed = args._speed;
    uint64_t _time = args._time;
  ...
  void destroy_tree(Node *root) {
    ...
    while (!s.empty()) {
      ...
      if (node->is_two) {
        ...
        pending_two[sali_get_thread_num()].push(node);
      } else {
        delete_items(node->items, node->num_items);
        delete_nodes(node, 1);
      }
  ...
  int scan_and_destory_tree2(
      Node *_subroot, std::vector<T> **keys,
      std::vector<P> **values,  // keys here is ptr to ptr
      bool destory = true) {
    std::list<Node *> bfs;
  ...
  int scan_and_destory_tree(Node *_subroot, std::vector<T> **keys,
                            std::vector<P> **values,  // keys here is ptr to ptr
                            bool destory = true) {
    std::list<Node *> bfs;
    std::list<Item *> lockedItems;
  ...
  restart:
    ...
    for (Node *node = root;;) {
                 ...
                 2)  // 2 means existing entry has data already
      {
        ...
        node->items[pos].comp.child = build_tree_two(
            key, value, node->items[pos].comp.data.key,
            node->items[pos].comp.data.value, node->speed / node->num_items,
            timeSinceEpochNanosec(), 1);
        /*if(node->speed / node->build_size==0){
          std::cout<<"speed == 0 origin_speed: "<<node->speed<<"build_size:
        "<<node->build_size<<std::endl;
        }*/
        insert_to_data = 1;
      ...
      } else  // 1 means has a child, need to go down and see
      {
        if (node->last_adjust_type == 3) {
          ...
          if (needRestart) goto restart;
          if (node->items[compress_pos].entry_type == 2) {
            node->items[compress_pos].upgradeToWriteLockOrRestart(versionItem,
                                                                  needRestart);
            if (needRestart) {
              goto restart;
            }
            node->items[compress_pos].entry_type = 1;
            node->items[compress_pos].comp.child = build_tree_two(
                key, value, node->items[compress_pos].comp.data.key,
      ...
      if (node->fixed == 0) {
        if (node->conflict_distribution(getGen())) {
          ...
          long double p_acc = (node->speed * (cur_time - node->build_time)) /
                                  (node->build_size + 1) +
                              (path_size - i) / (long double)128 + 0.1;
    ...
    if (node_prob_cur != nullptr) {
      ...
      long double speed = (long double)(numKeysCollected - prev_size) /
                          (cur_time - prev_build_time + 1);
      // std::cout << "speed: " << speed << " size_inc: " <<
      // std::to_string(numKeysCollected - prev_size) << "time: "<<
      // std::to_string(cur_time - prev_build_time) << std::endl;
      /*if(speed ==0){
        std::cout<<"speed == 0 size_inc: "<<std::to_string(numKeysCollected -
      prev_size)<<"time: "<<std::to_string(cur_time -
      prev_build_time)<<std::endl;
      }*/
      bulk_args args = {numKeysCollected, speed, cur_time, 1,
                        speed / prev_speed};
      ...
      retryLock:
        ...
        RT_DEBUG(
            "Final step of adjust, obtain parent %p lock OK, now give "
        ...
        if (cool_select_distribution(getGen())) {
          if ((!new_node->fixed) && new_node->num_items >= 64) {
            // std::cout << "push_to_cooling_pool, new_node: " << new_node <<
            // std::endl;
          ...
          if (allocated > memory_budget_) {
            std::cout << "allocated > memory_budget_, allocated: " << allocated
                      << std::endl;
  ...
  // SATISFY_LOWER = true means all the keys in the subtree of `node` are no
  // less than to `lower`.
  template <bool SATISFY_LOWER>
  int range_core_len(std::pair<T, P> *results, int pos, Node *node,
                     const T &lower, int len) {
      ...
      while (lower_pos < node->num_items) {
        if (node->items[lower_pos].entry_type == 2) {
          results[pos] = {node->items[lower_pos].comp.data.key,
                          node->items[lower_pos].comp.data.value};
          pos++;
        } else if (node->items[lower_pos].entry_type == 1) {
          pos = range_core_len<true>(
              results, pos, node->items[lower_pos].comp.child, lower, len);
        }
        if (pos >= len) {
          return pos;
        }
  ...
  // kSatisfyUpper = true means all the keys in the subtree of `node` are no
  // greater than to `lower`.
  template <bool kSatisfyUpper>
  int reverse_range_core(P *results, int pos, Node *node, const T &upper,
                         int len) {
    if constexpr (kSatisfyUpper) {
      int upper_pos = node->num_items;
      uint64_t versionItem;
      bool needRestart = false;
      while (pos < len && --upper_pos >= 0) {
        int prev_pos = pos;
      iter_prev_restart:
        int iterateRestartCount = 0;
        if (iterateRestartCount++) {
          pos = prev_pos;
          yield(iterateRestartCount);
          needRestart = false;
        }
        
        versionItem = node->items[upper_pos].readLockOrRestart(needRestart);
        if (needRestart)
          goto iter_prev_restart;

        if (node->items[upper_pos].entry_type == 2) {
          results[pos++] = node->items[upper_pos].comp.data.value;
        } else if (node->items[upper_pos].entry_type == 1) {
          pos = reverse_range_core<true>(
              results, pos, node->items[upper_pos].comp.child, upper, len);
        }

        node->items[upper_pos].readUnlockOrRestart(versionItem, needRestart);
        if (needRestart)
          goto iter_prev_restart;
      }
      return pos;
      ...
      int upper_pos = PREDICT_POS(node, upper);
      if (node->last_adjust_type == 3) {
        upper_pos = get_real_pos_from_compressed_node(node, upper_pos, upper);
      }
      versionItem = node->items[upper_pos].readLockOrRestart(needRestart);
      if (needRestart)
        goto restart;

      if (node->items[upper_pos].entry_type != 0) {
        if (node->items[upper_pos].entry_type == 2) {
          if (node->items[upper_pos].comp.data.key <= upper) {
            results[pos++] = node->items[upper_pos].comp.data.value;
          }
        } else {
          pos = reverse_range_core<false>(
              results, pos, node->items[upper_pos].comp.child, upper, len);
        }
      }
      node->items[upper_pos].readUnlockOrRestart(versionItem, needRestart);
